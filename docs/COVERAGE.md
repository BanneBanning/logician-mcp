...
> **Status (2026-08-28, v0.54.0):** all ten items shipped in some form the same day this audit landed — 18 new tools, 57 → 75. Live-verified: `logic_learn_key_command`/`logic_list_key_commands`, `logic_list_strips`/`logic_mixer_snapshot`, `logic_list_events`, record-arm (`logic_set_track_record_arm`), the `logic_load_instrument` mechanism, `logic_read_automation`, `logic_markers`, `logic_select_regions`, `logic_list_signatures` + the meter map in the bar math, `logic_split_region` (dialog-aware), and the metronome. **All six of the remaining writes were then run live on 2026-08-28 (v0.54.1)** and none of them is "shipped but unrun" any more: `logic_set_insert_bypass`'s press (active → bypassed → active, confirmed by `logic_list_inserts`), the bounce delivery-option writes (a WAVE / 16-bit bounce confirmed by `afinfo`, then put back to AIFF / 24-bit), `logic_bounce_in_place`'s OK (printed on a scratch copy, dry-print warning fired), `logic_remove_silence` end to end (preview `3 Regions` → apply → 3 regions), and `logic_export_stems` (two stems, 437356 frames each, solos restored). `logic_set_mixer` was run too and **the claim it existed for was disproved**: an open Mixer publishes every strip (`mixer_strips`, `Master` included) but does NOT put them within reach of the Accessibility strip tools, and it shadows the project window — the tool now says so, and `Master` stays on the `logic_mcu_*` plane. That run also found four bugs in shipped code: the Mixer window was undetectable (so the tool could neither verify an open nor close one), an open Mixer broke every Accessibility read, the bounce position converger assumed a constant meter and could not converge on this project's 5/4 section, and `logic_bounce_in_place` reported the muted SOURCE region as the print. All four fixed. Composability: U1/U2/U5–U10 fixed, U3/U4 covered by the key-command tools. Later the same day the pending six were live-verified (bounce delivery writes, bounce-in-place, remove-silence, stems, bypass write, set_mixer — whose Mixer-lifts-the-AX-limit hope was DISPROVEN and the guide corrected), and the below-the-line Region inspector experiment ran: full grammar for both region types, `logic_get/set_region_params` + `logic_rename_region` shipped (G06/G17/G25/G27-29). A class-B research batch followed: G52 CLOSED — the Smart Tempo mode is readable from File > Project Settings (roadmap item 1 done); G03/G11/G38/G39 shipped as `logic_track_info` + `logic_set_track_routing`; G49 shipped as `logic_tempo_events`. Eval-harness additions beyond the table: `logic_reset_to` (verified episode reset) and `logic_project_snapshot` (the one-call truth document). 57 → 83 tools. Details per item in FINDINGS (2026-08-28) and the tool reference.
