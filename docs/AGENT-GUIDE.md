...
**`logic_import_midi {at_bar, tracks: [{name, channel, notes: [...]}, ...]}`** writes the whole thing as a Standard MIDI File and lets Logic import it: bass, keys and a pad arrive together, on new tracks, in about ten seconds regardless of how long the music is. Same note vocabulary as `logic_record_midi`, widened to many tracks. Three things to know. Logic names the new TRACKS after whichever default patch it loads, so your names come back on the REGIONS — `logic_rename_track` afterwards if the track names matter, and `logic_load_instrument` to put the right sound on each one. It always creates NEW tracks: there is no way to import onto one that already exists. And it answers Logic's "Also import tempo information?" with **No** unless you ask otherwise, so the project's tempo map is untouched. Prefer `logic_record_midi` when the part must play through a specific existing track's instrument, or when the take should be audible as it happens; prefer this for everything else, including anything longer than a few bars.
