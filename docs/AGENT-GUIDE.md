...
**Shape a sound:**
`logic_add_plugin {track_name, plugin_name}` (mouse-free via the control-surface browser; exact catalog names like "Compressor", "Channel EQ") → `logic_list_inserts {track_name, route: "mcu"}` for the slot → `logic_list_plugin_parameters {track_name, insert_slot}` to read (page-capped; pass `max_pages` for giants) → `logic_set_plugin_parameter {track_name, insert_slot, parameter, target_value}` per change → or browse factory settings with `logic_plugin_preset` (`action: "list"` to see the names, `action: "select"` with `name` to load one).
...
Set ONE plugin parameter, verified by readback. ONE tool, TWO routes, and `route_used` says which ran. Pass window_title for the ACCESSIBILITY route: the parameter's formatted text field is written and read back, and the prior value is restored on verification failure; it needs expected_current_value (compare-and-set) and only reaches parameters logic_list_plugin_parameters marks `ax_writable`. Pass track_name + insert_slot for the CONTROL-SURFACE route: host automation (MCU vpot) with the LCD value echo as readback — it reaches EVERY plugin, custom-UI third-party included, needs no window, converges numeric targets adaptively, steps text targets (e.g. 'On', 'B') until exact match, matches the parameter against the MCU's abbreviated names ('Thrs' matches 'Threshold'), and rolls back on failed verification; expected_current_value is optional there. Give BOTH and a knob-only plugin no longer dead-ends: the write starts on the Accessibility route and falls back to the surface when the parameter publishes no field, reporting route_used 'mcu' and fallback_from 'ax'. Accepts headerless output/aux/bus strips — see STRIP ADDRESSING in the server instructions.
