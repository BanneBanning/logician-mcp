...
Logician controls Logic Pro through its **data plane**: the Mackie Control surface protocol over virtual MIDI (with Logic's own LCD/LED/fader echoes as verification), MIDI-note-bound key commands, timed MIDI streams for recording, and element-addressed macOS Accessibility for reading structure. **No coordinate-driven UI scripting.** A few verified fallbacks synthesize one of Logic's own shortcuts or press one hit-tested element, and each names its route in the result's `write_route` — trust that field rather than an assumption about how a write happened. (Three tools offer an explicit `allow_mouse: true` fallback that briefly takes the pointer; it is off by default, and `logic_set_track_stack` is the newest of them — its fold is an Accessibility press on the disclosure triangle, `write_route: ax_press`, and the click behind `allow_mouse` runs only if that press ever fails to move the triangle. No route takes the pointer *without* asking any more: `logic_set_cycle_range`'s ruler drag was the last, until 2026-09-03 — changing a cycle's LENGTH dragged the ruler with synthetic mouse events; it now writes both locators as numbers into the control bar LCD's own locator cells, `write_route` reads `lcd_locator_cells` on every call, and the drag is deleted. A unit test keeps the census of files that synthesize pointer input at two, so a third cannot appear quietly.) 

## First call, always

Run `logic_health` first. It starts the bridge daemon, checks every setup requirement, and returns a concrete fix string for anything missing. Requirements: Logic Pro running with a project open, Accessibility granted, a Mackie Control device in Logic bound to the "Logic MCP MCU" ports (one-time manual setup), and learned key commands (20 of them; learned automatically on first use — the Key Commands window flashes briefly; run `logic_setup_key_commands` once to do all learning up front instead).

## Core concepts

**1. Compare-and-set discipline.** *Some* write tools accept a compare-and-set argument — `expected_current_value`, `expected_current_volume_db`, `expected_current_pan`, `expected_current_mute`, `expected_current_solo`, `expected_current_bpm`, `expected_current_name`, `expected_current_bypassed`, or a whole `expected_current` object where several slots are written at once. **One guard per VALUE, never one guard for a tool:** `logic_set_track_mix` writes up to four settings in a call and takes four separate guards, so a refusal names which of them your belief was stale about, and the other three still go. **Which one, and whether there is one at all, is per tool: the schema is the answer**, and an argument a tool does not declare is refused (`invalid_arguments`), never silently ignored. Pass it whenever you know the current value: the tool reads the control, REFUSES with `precondition_failed` and writes nothing if reality disagrees, then converges, reads Logic's echo back and reports `before`/`after`. The writes with no such argument still verify by readback — they just cannot tell "the value was already wrong" from "you asked for this". Read before you write either way, and remember every value argument is ABSOLUTE (the single exception is `logic_set_track_mix`'s `relative_volume_db`).
...
**4. Bars and beats are 1-based; end bars are exclusive.** `start_bar: 5, end_bar: 9` = bars 5–8 (the range ends where bar 9 begins). Beats accept fractions (`beat: 2.5` = the off-beat after 2). Bar 1 = project start. Both **tempo changes and meter changes are followed**: the tempo map is read out of the Tempo List and the meter map out of the Signature List, and both are integrated (see 4b). Read the tempo and signature **at the playhead** with `logic_get_transport` — those two fields are the values in force WHERE THE PLAYHEAD IS, not project constants (the same project answers 121 BPM in 5/4 with the playhead at bar 51 and 120 BPM in 4/4 with it at bar 1), so read the whole tempo map with `logic_tempo_events` and the whole meter map with `logic_list_signatures`.
...
**7. Destructive operations have guards but Undo is your friend.** The four region editors — `logic_delete_region`, `logic_copy_region`, `logic_move_region` and `logic_split_region` — do not assume the selection is exclusive, they make it so: Logic's own project-wide `Deselect All` clears everything and is proven to have landed before the one named region is selected back, because Delete, Cut, Nudge and Split each act on every selected region in the project while the arrangement map only shows the rows Logic has rendered. Where that command is unavailable and rows are provably hidden they refuse and name them rather than editing blind, and every result says `selection_scope: "project"` or `"rendered_rows"` so you can see which claim you got. Their after-checks count the region total across every rendered row, not just the target track's, so a command that reached further than you asked comes back as a failure. `logic_delete_track` re-verifies the selection immediately before firing. All are restorable with `logic_trigger_key_command {name: "Undo"}` — but ONLY fire Undo right after a known edit (the menu shows no operation name; a blind Undo can revert something else).
...
**Mix moves:**
`logic_set_track_mix {track_name, volume_db?, pan?, mute?, solo?}` is ONE call for any subset of a strip's four settings — `{volume_db: -3, pan: 12}` is a single round trip, not two — plus `logic_add_send {destination: "Bus 1", level_db: -12}` (one call: a send without a level is silent) / `logic_mcu_set_send {send, level_db}` to change one afterwards / `logic_remove_send {send, destination}` to take one out again. Read-first, verified, and never a blind Undo. Each parameter gets its OWN section of the result (`volume`, `pan`, `mute`, `solo`) with that write's own state and readback, and the top-level `success`/`verified` are the AND of them; `written` / `unchanged` / `refused` say which went which way. **`volume_db` and `pan` are ABSOLUTE**, so "2 dB louder" is either a read followed by a write, or — better — `logic_set_track_mix {track_name, relative_volume_db: 2}`, which measures the offset from the value it reads immediately before moving the fader. Pass `expected_current_volume_db` / `expected_current_pan` / `expected_current_mute` / `expected_current_solo` whenever you already believe you know a current value: a stale belief then becomes a `precondition_failed` refusal for THAT parameter instead of a wrong write, and the rest of the call still lands.
...
`logic_select_regions {track_name, start_bar}` → `logic_list_events` for the region's actual notes (position, pitch, velocity, length). The Event List shows the SELECTED region only — an empty result means nothing is selected, not that the project has no MIDI. `logic_list_events` will do the selecting for you if you pass `track_name`. **It answers only the rows Logic has DRAWN.** A List Editors table publishes every row it holds and draws the cells of the rows in VIEW — measured, a 54-event region published all 54 and drew the 26 in the pane — so a list longer than the pane comes back short. The count you get (`event_count`, `marker_count`) is the list's own; `events_read` / `markers_read`, `unreadable_rows` and `unreadable_row_numbers` say how much of it is in the result, with a `warning`. Those rows are missing from the RESULT, not from the project: scroll the list in Logic (or make the pane taller) and read again before concluding a note or a marker is not there. `logic_markers {action: "list"}` reads the same way; `logic_list_signatures` REFUSES on an undrawn row instead, because a meter map that dropped a signature change would place every later bar confidently wrong.
...
**Deliver audio:**
`logic_render_track {track_name, start_bar?, end_bar?}` — dialog-free track export (32-bit float AIFF + optional bar-sliced WAV with RMS/peak metrics), ~8–9 s. It renders the whole PROJECT length whatever bars you ask for, and it moves the playhead to the project start and puts it back (reported in `playhead`). `logic_bounce_range` for the master, now with the delivery options the dialog actually has: `file_type` (AIFF/WAVE/CAF), `bit_depth`, `sample_rate` (`"48 kHz"`, `"48k"` and `48000` all work), `dithering`, `normalize`, `include_audio_tail`. Whatever you do not pass is left as the user set it, and the result's `delivered_as` says what the file really is. Those are the user's own settings and Logic keeps them — `options_changed` lists what you moved, and nothing puts it back.
...
1. `logic_project_snapshot {scope: "structure"}` — the BEFORE document (**~1 s** on the reference project once the session is warm; **~1.6 s** for the first one, which pays for the tempo and meter map reads it then caches).
2. …do the work…
...
Three things to hold on to. **`complete` is the field that matters on a snapshot**, not `success`, and it answers one question: is this document the whole project? It is false for either of two reasons, and they are different. A section that could not be read at all is present as `{"unavailable": "<reason>"}` and named in `unavailable_sections`, so a diff can never mistake a failed reader for an empty project. A section that read what Logic had RENDERED and can prove there is more — the track list behind a collapsed stack, the regions on those rows — is named in `partial_sections`, with the track numbers in `missing_track_numbers`; the reference project's collapsed stack alone hides ten of its twenty rendered tracks. Read the top-level `warning` first: it names the sections, the evidence and any section served from this server's map cache (`cached_sections`, and `verified: false` with it). **`logic_reset_to` discards unsaved work by design** — that is the whole tool; `confirm_discard: true` is required and there is no default, so save first if the work matters. **Pay for `scope` deliberately**: `structure` is ~1 s warm (~1.6 s for the first of a session) and Accessibility-only, `mix` adds two bank walks (~15 s on 25 strips), `full` adds ~10 s per track for inserts and sends. Diff at `structure` between episodes and take one `mix` snapshot when the mix itself is the thing under test.

## Listening to audio (IMPORTANT)

**The sound comes to you.** Every `logic_bounce_range` and `logic_render_track` result CARRIES its audio as an MCP audio content block — a file too long to fit one (over ~42 s, which is every full-track freeze render) carries a bounded WINDOW of its first seconds instead, and says so in `listen_note` and `audio_window`; the rest of it is one `logic_get_audio_clip` call away — and `logic_evaluate_change` carries BOTH versions (first block = baseline A, second = after B), so you hear the A/B in the same result you decide from. First-run handshake: after your first bounce, check whether an audio block reached you. If yes — listen, always. If the result arrived as text only, your client drops audio blocks: from then on open the returned `preview_path`/`clip_path` files with your client's FILE VIEWER (read-file capability), which most clients pass to the model as real multimodal audio (verified in Antigravity CLI). Never claim to have heard something you did not receive.
...
- **`verified`** — Logic's own feedback confirmed the new state (an LCD echo, a readback, a header checkbox). `success: true, verified: false` means the write went out but could not be confirmed: treat the value as unknown and re-read before building on it.
  ...
  - **One tool leaves it out entirely.** `logic_list_key_commands` reads the key-command registry FILE and never speaks to Logic, so there is nothing for Logic to have confirmed: the result carries no `verified` at all rather than a `true` that would mean nothing. Its own description says so.
...
- **A surface snapshot can be OLD, and it says so.** `logic_mcu_status` asks the bridge daemon's socket first, which computes the answer on the spot (`source: "socket"`, `age_seconds` ~0). When the daemon does not answer it falls back to the state FILE the daemon left on disk — and the daemon rewrites that file only when Logic sends something, so the mirror can be minutes old (117 s, 177 s and 197 s were measured in one sitting). That result carries `source: "state_file"`, its real `age_seconds`, and a `warning`; treat `assignment`, the faders, the LEDs and the LCD text in it as history, and `bridge_fix` as the way back. `online` is recomputed at READ time from when Logic actually last spoke, so it no longer inherits the file's frozen `true`.
...
- **`logic_trigger_key_command` verifies nothing about Logic's reaction — only that the bridge accepted the send.** A `success: true` means the MIDI note left this server, never that Logic acted on it: measured live, two identical `{name: 'Create Marker'}` calls returned byte-identical `success: true` while the marker count went 4 → 5 → 5, the second call a no-op Logic swallowed silently at an already-marked position — the tool's own result cannot tell the two calls apart. **A blind Undo is the sharpest case, not the only one.** `logic_trigger_key_command {name: "Undo"}` undoes whatever is on top of Logic's stack, which is not necessarily yours — in one session an Undo fired after a tool had *failed* removed an empty track someone else had just made, and nothing could tell it apart from a no-op. Fire Undo only when a tool reported `success: true` for the edit you want back, and re-read the arrangement map afterwards; to remove something you created, address it (`logic_delete_region`, `logic_remove_send`) instead. For every other command, verify its SPECIFIC effect yourself afterward by reading Logic back (a before/after count, the AX or LED state a command is known to change) rather than trusting the fire-and-forget result alone.
...
- **Nothing leaves this Mac through Logician — everything leaves through your CLIENT.** This server makes no network connections of its own: it drives Logic Pro locally over virtual MIDI and macOS Accessibility, and writes renders to `~/Library/Application Support/Logician/captures`. But every result it returns travels to whatever MCP client called it, and what those results carry is real material from the user's studio: project names and full file paths, track, region, marker, bus and plug-in names, and the rendered AUDIO itself — inline as base64 audio blocks by default, and fetchable by resource link either way. If that client is a cloud model, all of it goes wherever the client sends it, on the client's terms; this server can neither see that hop nor stop it. When a project is confidential, that decision belongs at the client end before the call — a local model, or a client configured not to forward audio — and `include_audio: false` on the rendering tools keeps the sound itself out of the reply (the paths still come back).
...
Two folds landed on 2026-09-03 (RELEASE-PLAN step 3, token audit cuts #4 and #5): five tool names became two, and the removed names are gone from `tools/list` — a call to one is refused with `Unknown tool`, which is the note this table exists to shortcut. Nothing lost a capability; every argument has a home on the survivor.

| you may remember | call this instead |
|---|---|
| `logic_set_track_volume {db, relative_db, expected_current_db, tolerance_db}` | `logic_set_track_mix {volume_db, relative_volume_db, expected_current_volume_db, tolerance_db}` |
| `logic_set_track_pan {position, expected_current_position}` | `logic_set_track_mix {pan, expected_current_pan}` |
| `logic_set_track_mute {enabled, expected_current}` | `logic_set_track_mix {mute, expected_current_mute}` |
| `logic_set_track_solo {enabled, expected_current}` | `logic_set_track_mix {solo, expected_current_solo}` |
| `logic_select_region {track_name, region_name, start_bar, exclusive}` | `logic_select_regions` with the same arguments — `mode` defaults to `region` |

The gain is not tidiness: four descriptions repeating one set of addressing, verification and compare-and-set rules cost 6,280 bytes of `tools/list` in every session, and the two region-selection tools 5,648 more. The four settings also now go out in ONE call, in a fixed order (volume, pan, mute, then solo — mute before solo, because Logic flashes the mute LED of every channel a standing solo silences and the readback would have to be taken through that blink window).

## Mechanism and measured costs

Twenty-one of the biggest tool descriptions end with a line pointing here. Each one keeps, in `tools/list`, only what changes what an agent DOES — what the tool is for, what it destroys, what it refuses, the one non-obvious argument rule, and the keys it promises back. **What is written HERE is the rest: how the thing actually works, and what it was measured to cost.** Nothing was deleted to make that split; every measured number below is the one the description used to carry, with its date where it had one. Read a tool's entry before you build a loop on it, when something is slower than you expected, or when a result field surprises you.

Costs are from the reference project (19 rendered tracks, 25 strips, 54 regions) unless another project is named, and every one of them is wall-clock against a live Logic Pro 12, not an estimate.

### `logic_edit_event`

Every cell in the Event List is a one-step-per-write stepper, so a big velocity or pitch move is many steps and costs a few seconds. The table RE-SORTS on every position and pitch write, which is why row numbers from an earlier read are stale. Logic's own 'Create new Event' places the note at the PLAYHEAD, so the tool parks the playhead and then corrects the created event's position fields.

### `logic_evaluate_change`

Three methods, 30–50 s each, because each is two renders. Method `render` cuts its two slices itself, so it first reads the project's tempo map out of Logic's Tempo List (`View > List Editors > Tempo`; 0.38–0.79 s cold, ~7 ms from the per-project cache, no playhead movement) and INTEGRATES it — exact for step tempo changes, and reported back in `tempo_map`. Only when the Tempo List cannot be read does it fall back to sampling the tempo at both ends of the range (parking the playhead), which is the case that can refuse: `bounce` and `solo_bounce` hand Logic the bar numbers and are never sampled.

Method `bounce` writes the parameter through the plugin WINDOW, which is why it needs an inspector showing the strip and a parameter marked `ax_writable`. The reference project's whole master chain — Channel EQ, Limiter, Sensor — is knob-only, so `bounce` cannot A/B it at all; `logic_bounce_range` plus a separate `logic_set_plugin_parameter` is the way round. `render` (freeze) and `solo_bounce` (solo) are track-only by nature. A solo this tool could not switch off again warns rather than staying quiet about it.

### `logic_get_region_params`

The Region inspector panel is opened — its 'More' section with it — and then LEFT open, because re-opening a collapsed panel costs 0.6 s and the next region call would only open it again; `panel_state` reports what was found and what was left, and the server closes them again when the session ends. When the Inspector PANE itself is hidden the call shows it, reads and presses it back, and since the pane is going away it closes the disclosures it opened on the spot rather than leaving them for a call that would have to show the pane again anyway (`panel_state.restore` then reads `settled` rather than `deferred`).

`region_type` is read off the rows Logic published rather than from the arrangement map: a MIDI region has Velocity Offset, Dynamics, Gate Time and the Q-rows, an audio region has Gain, Fine Tune, Fade-In/Out, Reverse and Smart Tempo.

### `logic_import_midi`

The file is valid by construction (every value is validated before a byte is written) and nothing is played in real time, so the cost is the DIALOGS, not the music: measured end to end, a single track lands in ~3.3 s and a 4-track arrangement in ~4 s, with note-level verification adding ~4 s and a travelling playhead up to ~7 s more. Sixteen bars cost the same as two, and so does a fourth track.

**Where it lands.** Logic imports at the bar line NEAREST the playhead, rounding rather than truncating (beat 3 of a 4/4 bar lands on the NEXT bar), so the tool parks the playhead exactly on `at_bar` first — that park is the single most expensive step of the call — and then verifies that every new region starts there.

**A routed entry** (`to_track`) costs a measured ~2.6 s per track, back to back against the same import unrouted: 5.8 s versus 3.3 s. The import still lands on a temp track, the region is moved onto the destination (~1.4 s, Cut/Paste with the same paste verification `logic_copy_region` uses), and the emptied temp track is deleted (~1 s, plus the re-read census).

**The tempo prompt.** Logic asks "Also import tempo information?" on EVERY import, before it has even parsed the file. Answered **No**, the project's tempo map is byte-identical afterwards (measured). Logic's "Don't ask again" checkbox is never touched.

**Verification.** `verify: 'events'` selects each new region by its track NUMBER, so it reads the region THIS call imported even though every unrouted import leaves another track called 'Studio Grand' behind; +3.9 s for one region. The generated `.mid` is a byte-for-byte function of the arguments — the same `tracks` regenerate it exactly — which is why it is removed again when the call ends rather than kept.

### `logic_list_regions`

The map is parsed from Logic's own accessibility descriptions. The name/mute split exists because of a measurement: with ONE track soloed, 53 of the reference project's 54 regions were renamed by this reader (2026-09-03) and every region tool then refused the name the map had just published.

`check_hidden_rows: true` costs a measured +40–50 ms on a 95–120 ms call. `selected` is omitted at false because it was 52 of 54 regions on the reference project, 17% of the payload. `type` is parsed from the region's help sentence, which Logic published for all 54 regions on 2026-09-02 and for only 2 of the same 54 hours earlier — hence `type_from: 'track_row'` and an absent field rather than a guess. Duplicate region names make verification count occurrences rather than identify a region, which is why every edit tool selects exclusively first.

### `logic_mcu_command`

**A press proves nothing on its own.** Measured 2026-09-02: MCU note 127 pressed six times returned `ok` and `success: true` every time with the surface byte-identical before and after. That is why `state` is `sent` and `verified` is false for the whole press family. `{"cmd": "status"}` on this same tool answers in 0.4–0.7 ms from the daemon's live in-process snapshot.

**The hold sweep, 2026-09-02.** Every hold from ~0.2 ms to 50 ms changed the view — 16 transitions out of 16 — with Logic's echo landing 102–106 ms later at every one of them, so the 50 ms the server used to wait was pure latency on every press, select, mute, solo and vpot_press. **The key-command sweep, 2026-09-03**: firing a marker-creating command at hold_ms 0, 1, 5, 10, 20 and 40 — 16 fires at 0 ms among them — created exactly one marker every time, zero duplicates, zero drops, so that plane's note-on/note-off gap defaults to 0 too. That covers 0–40 ms; the ~0.2 ms point the button sweep reached is not reachable through an integer field and was not measured there.

**Fader snapping.** Logic follows an absolute fader write (measured with and without the fader-touch note) but SNAPS the position to its own resolution: 5631 through 5635 all landed on 5628.

### `logic_mixer_snapshot`

Two bank walks — names and pan, then dB and LEDs — measured 2026-09-02 at 8.5–10 s on a 25-strip project, 10–11 s with a solo standing (which costs a banner fade), or 5.5 s with `include_record_arm: false` when no solo is standing. The surface is handed over in the channel-strip Volume view and the 3.7 s return to the pan view is deferred to the next tool that needs it, because this call had already read every byte it reports.

An armed strip's record LED flashes ~640 ms on / 640 ms off (measured), which is why every LED answer here is a sampling window rather than an instant. `any_soloed` is Logic's whole-project solo indicator, note `0x73`.

### `logic_plugin_preset`

`action: 'undo'` restores the parameter STATE rather than a name, which is how it recovers a plugin that was on no named setting at all — measured on a headerless strip, all eight of a Limiter's parameters came back exactly. It is **ONE undo per write**: four writes needed exactly four undos, and the per-plugin history is not monotonic (the state after undo #3 equalled the state after #1), so you cannot feel your way back. Diff `logic_list_plugin_parameters` against a capture taken BEFORE the first write.

`already_loaded_by_name` is a NAME match and never a check of the parameters — measured on a Channel EQ that Logic itself still ticked, 3 of 26 parameters sat away from the factory values. That is the behaviour that protects a tweak made on top of a setting; `reload: true` is how you override it.

### `logic_project_snapshot`

Costs per scope are in the episode recipe above: `structure` ~1 s warm and ~1.6 s for the first of a session (which pays for the tempo and meter map reads it then caches), `mix` adds two bank walks (~15 s on 25 strips), `full` adds ~10 s per track for inserts and sends.

It composes the same functions the individual tools call — `logic_get_transport`, `logic_list_signatures`' meter map, `logic_markers`, `logic_list_tracks`, `logic_list_regions`, `logic_list_strips`, `logic_mixer_snapshot`, `logic_list_inserts` route `mcu`, `logic_mcu_sends` — so each of their caveats applies here unchanged.

### `logic_read_automation`

The mechanism is the verification pass `logic_record_automation` already runs with the writing half removed: park the playhead at each sampled position and read the value Logic chases the lane to. It hands the control surface over in the view it read through (Volume, or the send/plugin view) instead of spending 3.3–4.6 s walking home to the Pan view; the next tool that needs Pan settles that itself, and a session that ends pays it at exit.

The sampling grid takes each bar's beat count from the project's Signature List (reported in `meter_map` / `meter_route`), not from the signature at the playhead, so a range crossing a 4/4-to-5/4 boundary asks for positions that exist. `automation_mode` is the inspector's own label, re-read up to four times before it gives up.

Cost: roughly a second per sampled position plus ~0.5 s of setup, plus a few ms of playhead travel each way when the playhead does not start inside the range (1–2.5 ms per bar since `7ddf884`, not the 5.1 s a 39-bar approach used to cost).

### `logic_record_automation`

**The pass.** Calibrate the control near the working range, switch the track to Latch over the control surface, roll playback placing calibrated moves at each musical moment, return to Read, restore the original value, and verify by REPLAYING the range while sampling Logic's own echo at every point.

**Cost**, measured 2026-09-02 on a 2-bar volume curve: 20.6 s with the playhead 38 bars away, of which 12.3 s is the range played through twice. `(bars + 1) × 2 × bar-seconds` is the floor, and the parking is charged for wherever you left the playhead. A volume curve also needs each distinct dB translated into an absolute fader position, which costs a converged write of ~5 s the first time that value is used; the taper map is cached per project and Logic build and cross-checked against the strip's own dB and fader echo before any of it is reused, so a repeated ride pays for one value or none (`calibration.source` says which, and a map that disagrees with the live pair is retired rather than trusted).

**Tempo map.** Each point's moment, the pre-roll bar and the per-point convergence budgets are integrated over the project's tempo map (0.38–0.79 s cold, ~7 ms from the per-project cache, no playhead movement, reported in `tempo_map`), so a curve across a tempo change lands on the beats asked for; without a readable map it falls back to one msPerBeat from the control bar. The verification is bar-based either way, so it is the proof.

**Why the roll anchor exists.** The schedule is timed from the OBSERVED crossing into the first point's bar, and that crossing counts only once the position display has MOVED off the parked pre-roll bar — because Logic plays from its own last play-start position, not from the playhead you parked. Measured 2026-09-03 on the sandbox: parked and verified at bar 1, at bar 20 and at bar 1 again (once through a Go to Beginning locate), playback began at bar 9 all five times, and a curve asked for at bars 2–4 landed at bars 9–11 with the verification replay confirming it there. That is now a refusal with nothing written, and `roll_anchor` names the bar the schedule was actually timed from.

### `logic_record_midi`

**The cost model, measured.** ~7 s of fixed overhead plus (the take's bars + ONE pre-roll bar) × bar-seconds, plus ~5 s more when the verification render runs, plus a few ms even when the playhead starts far from the take (the locate is 1–2.5 ms per bar since `7ddf884`, not the ~10 s a distant playhead used to add) and ~3 s on the first call of a session while the tempo, meter and bank caches fill. A two-note one-bar take measured 15.4 s warm before this was profiled and reads ~9 s now.

**The pre-roll.** One bar leads into `start_bar` and the stream starts on the observed MCU-timecode crossing into it, so Logic's count-in settings do not affect CORRECTNESS — but they do affect cost, and the tool reads the flag rather than paying for it blind: with the count-in ON the playhead parks ON `start_bar` and the count-in bar does the leading in, which is one bar of wall clock instead of two; with it off the tool parks one bar early itself. `pre_roll_route` says which ran. The pre-roll bar's beat count and beat length come from the project's own Signature List and Tempo List AT THAT BAR, not from the control bar's reading at the playhead, so a take at bar 2 does not time differently because the playhead was left in a 5/4 bar 39 bars away.

**Which sync branch ran is reported, because the two do not time the same.** `sync_branch: 'beat_edge'` means the pre-roll bar's last beat was seen, and the lead to `start_bar` is READ off the sub-beat digits of the very repaint that revealed it (`sync_lead_route: 'display_sub_beat_position'`, with `sync_position` naming it) rather than assumed to be a whole beat — assuming it put five measured takes anywhere from 22.9 ms early to 23.4 ms late on identical arguments, because the edge is seen when Logic next repaints and not when it happens, while reading it put three inside a 4.2 ms band. Where Logic blanks those digits the lead is the nominal beat again and the route's measured latency constant is subtracted (46 ms inside Logic's count-in bar, 23 ms inside one this tool parked), which centres that band rather than removing it, and `sync_note` says so. `'bar_line'` means that beat was missed, there was no lead to schedule into, `sync_compensation_ms` does not apply, and the notes land ~39 ms late (measured).

**`sync_compensation_ms`.** Default 18 ms when the display published the sub-beat position the lead was computed from — measured 2026-09-02, three takes with nothing subtracted landed 16.1 / 17.2 / 20.3 ms late (a 4.2 ms band against the 46 ms one an assumed beat gives), and two at the 18 ms mean of those landed 0.0–2.6 ms off the beat. When Logic blanked those digits the lead is an assumed whole beat and the default is the latency measured for that pre-roll route: 46 ms inside Logic's count-in bar, 23 ms inside one this tool parked (the single 45 ms default that shipped before put those 21.5 ms early, 2/2 takes). It does nothing on the bar-line branch — there is no lead there to subtract it from.

**The result's counts.** `events_streamed` counts events the way Logic's Event List does, so it can be diffed against `logic_list_events`; `midi_messages_streamed` is the raw wire count. `recorded_end_bar` is past `end_bar` because Logic keeps recording until the transport stops, which is 600 ms after the take's last event. Deleting your own take with `logic_delete_region` measured 0.7 s, with `regions_before`/`regions_after` as evidence.

**Tempo map.** Note offsets are integrated over the project's tempo map (0.38–0.79 s cold, ~7 ms cached, no playhead movement, reported in `tempo_map`), so notes land on the grid across a tempo change. When the Tempo List cannot be read, placement falls back to constant-tempo bar math and the tempo is sampled at the take's first and last bar instead (playhead parked, read, restored — once per call, shared with the verification render); differing readings then produce a `warning`.

### `logic_remove_automation`

Logic's own command is `Mix > Delete Automation > Delete All Automation on Selected Tracks`, which is where TRACK-WIDE comes from. Two guards run before the press: the track selection is narrowed to that ONE row (the menu item says Selected Tracks and means it), and Logic is brought to the front — a menu press from the background answers success and does nothing, measured 2026-09-03.

The control is left where the automation last put it: measured 2026-09-03, a track sitting at −5.1 dB read −2.9 dB once its curve was deleted. A mixer-only strip is out of reach because the command acts on selected TRACKS — measured on the sandbox, `Aux 1` has no track row and its lane could not be cleared this way.

**Cost**, measured 2026-09-03: 9.1 s for a verified removal proved at three sampled positions, of which the menu press is 0 ms and essentially all the rest is the two verification reads (~1 s a sampled position plus the playhead travel to reach them); 3.6 s for the `already_empty` answer, which pays for one read and no press. `verify_resolution_beats` defaults to one sample a bar because presence-or-absence needs far less resolution than shape.

### `logic_render_track`

**The mechanism.** Select the track, toggle freeze over the 'Logic MCP Commands' MIDI port, press play (Logic then renders the whole track offline, typically seconds), copy the 32-bit float AIFF out of `Media/Freeze Files` to the captures folder, and unfreeze again. Freeze mode is Pre Fader, so the render is the full track from project start including all plugins and automation. ~8–9 s on the reference project, of which 4.3–4.9 s is Logic's own freeze engine.

The playhead restore locates at 1–2.5 ms per bar of distance since `7ddf884` — it was ~0.1 s/bar, which used to turn an 8.2 s render into 11.8 s from 55 bars out.

**Why the audio is a window.** A full-track freeze render is 125.6 s of audio on the reference project, far past the 400 KB an MCP audio block may carry, so a bounded ~42 s window rides along and the rest is one `logic_get_audio_clip` call away.

**Tempo.** With `start_bar`/`end_bar` the slice's boundaries are integrated over the project's tempo map, read out of Logic's Tempo List with no playhead movement and reported in `tempo_map`. That read costs 0.38–0.79 s (tempo) / 0.23–0.66 s (meter) the first time this server sees a project and 27–48 ms every time after — both maps are cached ON DISK per project, so a fresh session is not a cold map. When the Tempo List cannot be read the slice falls back to constant-tempo bar math and the tempo is sampled at both ends of the range instead (parks the playhead, reads, restores), with a `warning` naming both readings when they differ. The FULL render is unaffected either way, and without a bar range no tempo is read at all.

### `logic_select_regions`

The command modes fire Logic's own key commands, spelled as Logic 12.3.1 spells them: 'Select All Regions/Cells of Same Track', 'Select All Following', 'Select All Following of Same Track/Pitch', 'Select All', 'Deselect All'. One missing from the registry is learned on the spot, which is the consent note in the result.

**Key focus.** Logic's Tracks-area keyboard focus is probed before the command fires and repaired if it is elsewhere — for `all` and `none` against the track that is already selected, so nothing else moves — because a selection command fired with the focus in another part of Logic does nothing at all, silently. When the focus cannot be proven, the failure names it as the first suspect. A focus held by ANOTHER Logic window cannot be repaired from here at all, so nothing is attempted and no time is spent trying.

### `logic_set_cycle_range`

Both locators are stepped one unit per `AXValue` write, on the same control bar LCD engine the playhead display uses: measured 2026-09-03, **0.1–1.6 ms per step, 7 ms for both locators of a 15-bar move, 72 ms for the whole round trip** when the LCD is already showing its locator cells. When it is not, the display-mode switch dominates — ~200 ms each way, plus the wait for Logic to rebuild the LCD's children after the switch back (without that wait a `logic_get_transport` fired straight afterwards reads a half-built control bar and reports a null key signature) — so the call costs **0.7–1.2 s**. A cell is recognised by its shape and its digits (`0005  1  1  001`), not by a translated word, so an LCD that already shows the locators needs no English at all.

**Why there is no drag any more.** Until 2026-09-03 a LENGTH change dragged the ruler with synthetic mouse events. That route took the user's pointer, engaged Cycle mode by itself, scrolled the ruler out from under the next call, needed the range on screen, and borrowed the playhead — parking it on up to five candidate bars to work out which bar the cycle region sat on. It had also stopped working: three consecutive length changes refused with *"hit test at the cycle strip of the ruler resolved to `AXTable ''`"*, because a drag aims at screen coordinates and cannot see what the window stack has done to them. Numbers have none of those failure modes, and the pixels-per-bar, bar-line-anchoring and ruler-recentring machinery the drag needed is deleted with it.

**The one hazard the write order exists for.** An inverted locator pair is Logic's SKIP cycle — a different feature, which publishes an empty size description on the ruler's cycle region and reads `4` on the Cycle button. The two legs are therefore ordered so the pair is never inverted, not even between them.

### `logic_set_plugin_parameter`

The control-surface route is host automation (an MCU vpot) with the LCD value echo as readback: it converges numeric targets adaptively and steps text targets (e.g. 'On', 'B') until they match exactly. The Accessibility route writes and reads back the parameter's formatted text field and restores the prior value on verification failure. Concept 10 above has the measured comparison: ~0.9 s cold and ~0.5 s warm for the one-call form, against ~15.8 s for the list-then-read-then-write chain it replaces.

### `logic_set_region_params`

Every write goes through the Region inspector, which is why a hidden Inspector pane is shown, written through, read back and pressed away again rather than refused. `scope: 'selection'` is measured rather than assumed: two selected regions both took the write.

### `logic_set_track_mix`

`led_evidence` says which sampling window was paid for the mute read: `blink_window` 1.4 s while a solo stands, `settled_window` 0.3 s otherwise. That is the same blink Logic uses on every channel a standing solo silences, and the reason solo is written last in the fixed order.

### `logic_set_track_routing`

**The menu press is intermittent.** Logic sometimes answers it with success and opens nothing, so the tool retries three times, clearing Logic's pending UI state with an Escape between attempts, and looks for the menu three ways — over the slot itself, then two walks of Logic's tree — because Logic parents the menu somewhere no walk can reach on a strip whose output is 'No Output'. Measured live 2026-09-03: that state used to be a one-way door, 21 s per failed attempt and no way back through this tool; it now routes back in ~2 s, and `menu_route` says which look found the menu. The output slot was the reliable one in testing; the input and group slots hit this more often.

Pressing a menu CATEGORY rather than a destination opens its submenu and routes nothing, which used to cost 10 s and a misleading failure — hence the refusal that names what to use instead.

### `logic_split_region`

The command is Logic's own 'Split Regions/Events at Playhead Position', and the modal a crossing note raises is 'Notes Crossing Split Point'. The exclusivity receipt is watched rather than assumed: the rendered selection count is observed falling to zero after 'Deselect All' before the one named region is selected back. The playhead park sets the sub-beat fields of the control bar's position display as well, which `logic_set_playhead` does not, because a 'verified' bar/beat can still sit almost a whole beat late and for a split that is a wrong cut rather than a rounding error.

### `logic_tempo_events`

**Two parts of the write are surprising.** Logic's own 'Create new Tempo Event' button places the event AT THE PLAYHEAD, not on a bar line, and parking the playhead only sets its bar and beat — the division and tick it already carried come along — so the tool creates the event and then steps the new row's own position fields back to the exact bar/beat asked for. And the BPM is stepped on the ROW's own tempo cell, one BPM per write, NOT through the control bar's tempo slider: on a project that already has a map, that slider answers a tempo write with a modal alert ("Multiple Tempo Events detected!") that freezes the whole Accessibility plane until someone clicks OK.

**Cost.** A `list` is ~7 ms from the per-project cache and 0.4–0.8 s when it has to read the list; a write is a couple of seconds, dominated by reading the whole Tempo List twice (once before, once to verify) rather than by the playhead, which travels to any bar in milliseconds. The whole edit is held inside ONE List Editors pane cycle instead of the three or four it used to open and close, which roughly halved every write (measured 2026-09-03).

After a write the server's cached tempo map is dropped so every later bar→seconds conversion re-reads it; the cached METER map is left alone, because a tempo event is not a time signature and re-reading the Signature List cost the next `logic_list_signatures` a full second for nothing.

## Tool reference

All 81 tools, generated from the live server schemas (v1.0.0-beta.1) by
...
Which tool does X? Ask this first - 0.4-1.0 ms, no contact with Logic - before reading a schema file, writing your own probe, or reaching for logic_mcu_command's raw escape hatch. Search every tool this server has by keyword and get the matching typed definitions back: name, description, full input schema and safety annotations. Use it when you know what you want to do but not which tool does it, or when a tool you expected is not being offered. The search covers the whole registry even when `--toolsets` narrows a session, and each match says which toolsets hold it and whether it is active here.
...
Parameters:
  ...
  - `include_audio` (boolean): Attach the rendered audio as MCP audio content blocks (default true) — see AUDIO RESULTS AND LISTENING in the server instructions before passing false.
...
Parameters:
  ...
  - `region_name` (string): The region's own name — accepts either the clean name or the muted spelling Logic publishes, see REGION NAMES in the server instructions.
  ...
  - `track_number` (integer): Row number when several tracks share the name — see TRACK ADDRESSING in the server instructions.
...
Parameters:
  ...
  - `expected_project_path` (string): Refuse unless this is the open project.
  - `include_audio` (boolean): Attach the rendered audio as MCP audio content blocks (default true) — see AUDIO RESULTS AND LISTENING in the server instructions before passing false.
  - `insert_index` (integer): Method 'bounce' only. Accessibility ordinal, NOT the Mackie insert_slot — see INSERT NUMBERING in the server instructions.
  - `insert_slot` (integer): Required for methods 'render' and 'solo_bounce'. Mackie physical slot 1-8, NOT the Accessibility insert_index — see INSERT NUMBERING in the server instructions.
...
Parameters:
  ...
  - `speed` (number): Record at speed x tempo (1-8, default 1) and scale event times: the same bar positions in a fraction of the wall time, at the cost of timing precision and a chipmunked monitor.
  - `start_bar` (integer): Recording start bar (>= 2); default = the earliest event's bar.
  - `sync_compensation_ms` (number): Timecode display latency subtracted in the beat-edge sync only; lower it if notes land late, raise it if they land early, and leave it out for the measured default (see the guide).
  ...
  - `track_number` (integer)
  - `verify_render` (boolean): Default true: freeze-render the recorded bars afterwards and return slice metrics as proof (it moves the playhead again and restores it, so `playhead` in the result describes its outcome).
...
Parameters:
  ...
  - `at_bar` (integer): The bar the arrangement's first bar lands on, default 1; the playhead is parked exactly here first (the call's most expensive step) because Logic imports at the bar line NEAREST the playhead. Every event's `bar` must be this bar or later.
  - `expected_project_path` (string)
...
Parameters:
  ...
  - `insert_index` (integer): Needed only when the same plugin sits in several slots. Accessibility ordinal, NOT the Mackie insert_slot — see INSERT NUMBERING in the server instructions.
  ...
  - `reload` (boolean): For action 'select', default false. Press the entry even when the plugin window's header already names it — the way to get a setting's OWN values back after tweaks were made on top of it. The default false is a name-match no-op (state "already_loaded_by_name") that leaves those tweaks alone; reload: true OVERWRITES every parameter of the plugin, exactly as any other load does.
...
Parameters:
  ...
  - `region_name` (string): With track_name: which region. The region's own name — accepts either the clean name or the muted spelling Logic publishes, see REGION NAMES in the server instructions.
...
Parameters:
  ...
  - `region_name` (string): Which region (its current name), with start_bar to disambiguate. The region's own name — accepts either the clean name or the muted spelling Logic publishes, see REGION NAMES in the server instructions.
  - `start_bar` (integer): The region's current start bar.
...
Parameters:
  ...
  - `hold_ms` (integer): How long a press/select/mute/solo/vpot_press - or a keycmd - holds the button down; the default 0 is the measured one on both planes, so pass a hold only for Logic Control's own hold behaviours, which remain unswept.
...
Set the cycle (loop) locators to a whole-bar range, e.g. bars 5-9 - written as NUMBERS into the control bar LCD's own locator cells, with NO MOUSE anywhere on this path. Changing a cycle's LENGTH takes the pointer no more than moving it does: the ruler drag this tool used until 2026-09-03 is deleted, and 'write_route' reads 'lcd_locator_cells' on every call (the old 'ax_position_grid_snap' and 'cg_drag_create' are gone). Nothing has to be visible - a range past the edge of the ruler needs no scrolling - and the playhead is never borrowed or moved. ONE VISIBLE SIDE EFFECT, undone before the call returns: when the LCD is not already showing its locators it is switched to the display mode that carries them and switched back, and 'display_mode' reports what was found and whether it went back. VERIFIED ON TWO PLANES: both locator cells read back the requested bars exactly, on beat 1 division 1 tick 1, and the ruler's own cycle region reports the requested bar count ('ruler_witness', which says why it is unavailable rather than dropping the key). A range already set comes back 'already_set' with nothing written. A write that fails verification is put BACK to the range this call read first, by the same route, verified ('Restored: true') or quoting where it is left. Cycle mode is left exactly as it was found, with 'cycle_enabled_before' and 'cycle_enabled' both reported; 'enabled' turns it on or off afterwards. REFUSED with nothing written: a bar past the end of the project (Logic clamps the locator at the project's last bar, and the refusal names where it stopped); an LCD that will not show its locator cells (the message names the one-time 'Customize Control Bar and Display' setting that fixes it - there is no mouse fallback, on purpose); and an LCD showing more locator pairs than the ruler can tell apart (Punch locators beside the cycle ones). Mechanism and measured costs: see the guide.
...
Expand or collapse a track stack, verifying the new state and reporting which subtracks were revealed or hidden. Fails with not_exposed if the track is not a stack. Subtracks of a collapsed stack are otherwise invisible to logic_list_tracks and logic_select_track. MOUSE-FREE: the write is an Accessibility press on the stack's own disclosure triangle, addressed as an element - no pointer, no key command, and Logic does not even have to be in front (measured 2026-09-03: 22-40 ms, both directions, with another app frontmost). `write_route` says ax_press. A stack Logic has scrolled out of sight is scrolled back first, because a header drawn off-screen cannot be written to at all; that reports `scrolled_into_view` and moves the track selection. If the press ever fails to fold the stack the call REFUSES rather than reaching for the pointer, and it says whether Logic carried the press out: one that returns in under a millisecond (`ax_press_ms`) means Logic's Accessibility ACTIONS have gone inert app-wide, which affects every element-addressed write, not just this one. allow_mouse: true then permits a hit-test-guarded synthetic click on the triangle - not an Accessibility action, so it still lands in that state - and the result carries `ax_press_inert` plus a warning that the pointer moved.

Parameters:

  - `allow_mouse` (boolean): Permit the synthetic-click fallback if the mouse-free Accessibility press does not fold the stack. Default false: the call refuses instead and names this argument.
...
Parameters:
  ...
  - `insert_index` (integer): Required when the same plugin occupies several slots. Accessibility ordinal, NOT the Mackie insert_slot — see INSERT NUMBERING in the server instructions.
...
Parameters:
  ...
  - `insert_slot` (integer): Names the plugin on the control-surface route. Mackie physical slot 1-8, NOT the Accessibility insert_index — see INSERT NUMBERING in the server instructions.
...
Parameters:
  ...
  - `insert_index` (integer): 1-based. Accessibility ordinal, NOT the Mackie insert_slot — see INSERT NUMBERING in the server instructions.
...
Parameters:
  ...
  - `insert_slot` (integer): Names the plugin on the control-surface route by physical slot, when you already know it or need to pick between two copies of the same plugin. plugin_name is the cheaper way in and costs no extra call. Mackie physical slot 1-8, NOT the Accessibility insert_index — see INSERT NUMBERING in the server instructions.
...
Parameters:
  ...
  - `max_steps` (integer): How many catalog ENTRIES to look at before giving up, default 700 — entries actually shown, not messages sent. A wall-clock budget scales with it (15 s at the default), so a fruitless search stops in seconds instead of minutes and raising this really does buy a longer search. The list holds every installed instrument in every channel format and is grouped by vendor, not alphabetical, so a 'never showed' refusal reports how much of the catalog it got through and which bound it hit.
...
Parameters:
  ...
  - `insert_slot` (integer): Required when parameter is 'plugin'. Mackie physical slot 1-8, NOT the Accessibility insert_index — see INSERT NUMBERING in the server instructions.
  - `max_points` (integer): Cap on sampled positions, default 64. Exceeding it widens the step; the range is never truncated, and end_bar is sampled either way. The one exception is max_points: 1, which leaves room for start_bar only and warns that end_bar was not sampled.
