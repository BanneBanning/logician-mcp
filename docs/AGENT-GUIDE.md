...
Logician controls Logic Pro through its **data plane**: the Mackie Control surface protocol over virtual MIDI (with Logic's own LCD/LED/fader echoes as verification), MIDI-note-bound key commands, timed MIDI streams for recording, and element-addressed macOS Accessibility for reading structure. **No coordinate-driven UI scripting.** A few verified fallbacks synthesize one of Logic's own shortcuts or press one hit-tested element, and each names its route in the result's `write_route` — trust that field rather than an assumption about how a write happened. (Two tools also offer an explicit `allow_mouse: true` fallback that briefly takes the pointer; it is off by default. Two routes take the pointer *without* asking, and both say so in `write_route`: `logic_set_track_stack`'s hit-tested click on the disclosure triangle — the only route Logic leaves, since it refuses to learn a key command for its own stack fold — and `logic_set_cycle_range`'s ruler drag when the cycle LENGTH changes.)
