...
TAKES REAL WALL-CLOCK TIME, and the model is measured: ~7 s of fixed overhead plus (the take's bars + ONE pre-roll bar) x bar-seconds, plus ~5 s more when the verification render runs, plus up to ~10 s when the playhead starts far from the take (~0.13 s per bar of travel, each way) and ~3 s on the first call of a session while the tempo, meter and bank caches fill. A two-note one-bar take measured 15.4 s warm before this was profiled and reads ~9 s now. THE SIBLING: logic_import_midi writes a whole multi-track arrangement as a Standard MIDI File and lets Logic import it - byte-exact, seconds rather than minutes, and free of the Smart Tempo hazard below - and its `to_track` puts the material on an EXISTING track too, by moving it there after the import; this tool is the one that PERFORMS a part through an existing track's instrument in real time, so the take is audible as it happens. Composes MIDI into the project with ZERO dialogs and no files: notes are streamed in real time over the dedicated 'Logic MCP MIDI In' port while Logic records them onto the selected software-instrument track (one bar leads into start_bar and the stream starts on the observed MCU-timecode crossing into it, so count-in settings do not affect CORRECTNESS - but they do affect cost, and the tool reads the flag rather than paying for it blind: with Logic's count-in ON the playhead parks ON start_bar and the count-in bar does the leading in, which is one bar of wall clock instead of two; with it off the tool parks one bar early itself. pre_roll_route says which). WHICH SYNC BRANCH RAN IS REPORTED, because the two do not time the same: sync_branch 'beat_edge' means the pre-roll bar's last beat was seen, and the lead to start_bar is READ off the sub-beat digits of the very repaint that revealed it (sync_lead_route 'display_sub_beat_position', sync_position naming it) rather than assumed to be a whole beat - assuming it put five measured takes anywhere from 22.9 ms early to 23.4 ms late on identical arguments, because the edge is seen when Logic next repaints, not when it happens, while reading it put three inside a 4.2 ms band. Where Logic blanks those digits the lead is the nominal beat again and the route's measured latency constant is subtracted (46 ms inside Logic's count-in bar, 23 ms inside one this tool parked), which centres that band rather than removing it, and sync_note says so; 'bar_line' means that beat was missed, there was no lead to schedule into, sync_compensation_ms does not apply, and the notes land ~39 ms late (measured). The pre-roll bar's beat count and beat length come from the project's own Signature List and Tempo List AT THAT BAR, not from the control bar's reading at the playhead - so a take at bar 2 no longer times differently because the user left the playhead in a 5/4 bar 39 bars away. Creates a normal recorded region. THE RESULT NAMES IT: created_region {track_name, track_number, start_bar, end_bar, recorded_end_bar, delete_with} - remove your own take with logic_delete_region (measured 0.7 s, with regions_before/regions_after as evidence), NOT with Undo. recorded_end_bar is past end_bar on purpose: Logic keeps recording until the transport stops, which is 600 ms after the take's last event. events_streamed counts events the way Logic's Event List does, so it can be diffed against logic_list_events; midi_messages_streamed is the raw wire count. By default the result is verified with a dialog-free freeze render of the recorded bars (non-silent metrics prove the notes landed and sound through the instrument); that render skips the audio encodes this tool would only discard, so to HEAR the take pass verification.rendered_slice to logic_get_audio_clip. SMART TEMPO GUARD: a project tempo mode of ADAPT (or AUTO, which can resolve to Adapt) makes Logic rewrite the project's TEMPO MAP to follow the recording, so this refuses before arming and names the fix; when the mode cannot be read off the control bar the recording proceeds and the result carries a warning saying it went unverified. TEMPO MAP: note offsets are integrated over the project's tempo map, read out of Logic's Tempo List (View > List Editors > Tempo; ~2 s, no playhead movement, cached per project and reported in tempo_map), so notes land on the grid across a tempo change. When the Tempo List cannot be read, placement falls back to constant-tempo bar math and the tempo is sampled at the take's first and last bar instead (playhead parked, read, restored — once per call, shared with the verification render); differing readings then produce a `warning`. Either way speed > 1 is REFUSED with precondition_failed on a non-constant tempo: speed mode overwrites the tempo slider and restores a single value, which cannot put a tempo map back. Real-time recording (speed 1) touches no tempo and stays available. MAY RETURN `warning` — read it before the rest of the result (RESULT CONTRACT in the server instructions).
...
Parameters:
  ...
  - `sync_compensation_ms` (number): Timecode display latency subtracted in the BEAT-EDGE sync only. Default 18 ms when the display published the sub-beat position the lead was computed from - measured 2026-09-02: three takes with nothing subtracted landed 16.1 / 17.2 / 20.3 ms late (a 4.2 ms band against the 46 ms one an assumed beat gives), and two at the 18 ms mean of those landed 0.0-2.6 ms off the beat; when Logic blanked those digits the lead is an assumed whole beat and the default is the latency measured for that pre-roll route - 46 ms inside Logic's count-in bar, 23 ms inside one this tool parked (the single 45 ms default that shipped before put those 21.5 ms early, 2/2 takes). Lower it if notes land late, raise it if they land early. It does nothing on the bar-line branch - there is no lead there to subtract it from - and the result says which branch and lead route ran and what was applied.
...
Parameters:
  ...
  - `verify` (string): 'census' (default): prove the import by the track/region diff - fast. 'events': additionally select each new region (by track NUMBER, so a project full of same-named 'Studio Grand' tracks still reads the right one) and diff its notes against what was written, reporting per-track counts and mismatches (measured +3.9 s for one region). A region whose notes could not be read comes back `verification: 'unverified'` with the reason, not as a mismatch. One of: `census`, `events`.

#### `logic_plugin_preset`

Browse and load a plugin's settings (presets). action 'list' enumerates the setting menu — every name, its category, and which one is marked as loaded — WITHOUT changing anything; action 'select' loads one by name (bare 'Rock Bass' or qualified '03 Guitars/Rock Bass'), verified against the plugin window's setting label; action 'step' walks next/previous N settings via Logic's topmost-plugin-window key command, the only route that needs no readable menu; action 'undo' presses the setting menu's own Undo, which is the ONLY way back from a load — it restores the parameter state rather than a name, so it also recovers a plugin that was on no named setting at all (measured on a headerless strip: all eight of a Limiter's parameters came back exactly). The plugin window is opened and closed again if this call opened it. Reading the menu needs Logic frontmost for a moment (a menu cannot open in a background app). Honesty contract: 'list' returns presets: null plus a reason when the plugin's UI exposes no Logic setting pop-up (fully custom UIs), and presets: [] — an empty list, not a failure — for plugins that genuinely ship no factory settings; 'step' reports success: false when the label did not move. A 'select' whose setting the header ALREADY NAMES presses nothing and comes back state "already_loaded_by_name" — a NAME match, never a check of the parameters (measured on a Channel EQ that Logic itself still ticked: 3 of 26 parameters sat away from the factory values), which is what protects a tweak made on top of the setting; pass reload: true when you need the setting's own values loaded and verified. WARNING: loading a setting overwrites EVERY parameter of the plugin, and a setting name is not a promise about the current state — unnamed tweaks on top of a named setting are lost and re-selecting the old name does not bring them back. Use action 'undo' to get back, ONE call per write you want to take back (measured: four writes needed exactly four undos) — and diff logic_list_plugin_parameters against a capture taken BEFORE the first write, because the per-plugin history repeats states and this tool cannot tell you where in it you are. ACCESSIBILITY-ONLY: a headerless strip works only while an inspector is SHOWING it — see STRIP ADDRESSING in the server instructions. MAY RETURN `warning` — read it before the rest of the result (RESULT CONTRACT in the server instructions).
...
Render ONE track offline to an audio file ON DISK with ZERO dialogs, via Track Freeze. NOT bounce-in-place: it produces a FILE, it does not commit a new audio region into the project, so it is not the tool for 'print that so I can chop it' - resampling a part back into the arrangement has no route in this server yet. Mechanism: selects the track, toggles freeze over the 'Logic MCP Commands' MIDI port, presses play (Logic then renders the whole track offline, typically seconds), copies the 32-bit float AIFF out of Media/Freeze Files to the captures folder, and unfreezes again. ~8-9 s on the reference project, of which 4.3-4.9 s is Logic's own engine. Requires 'Toggle Track Freeze' in the key command registry and the MCU bridge running. THE PLAYHEAD MOVES AND IS PUT BACK: the render jumps it to the project start and rolls from there, so where you had it is read first and restored afterwards, verified against Logic's control bar and reported in `playhead` — that restore steps the control bar bar by bar (~0.1 s per bar of distance measured, so a playhead 55 bars from the project start turns an 8.2 s render into 11.8 s), and if it fails the result WARNS and names the position the playhead was left at. LISTENING: a full-track freeze render is the whole PROJECT long whatever bars you ask for (125.6 s on the reference project), which is far past the 400 KB an MCP audio block may carry — so what rides along is a bounded WINDOW of the first ~42 s, named in `audio_window` and in `listen_note`, with the complete file at `path`, an AAC copy at `preview_path` and any other stretch one logic_get_audio_clip call away. A short render is carried whole; with start_bar/end_bar the SLICE is what gets previewed and attached. A result never arrives audio-less in silence: when no block can be made, `listen_note` says why and where to listen instead. DISK: every call writes ~46 MB of project-length audio into the captures folder, and a retention policy shared by every tool that writes there keeps the newest 200 captures within 2 GB, reporting anything it removed in `captures_pruned`. WHAT `verified` MEANS HERE: it reports the CLEANUP, not the render. `verified` aliases `unfrozen`, so a perfectly good render whose freeze could not be undone comes back verified: false with the track still frozen in Logic (unfreeze it there). Judge the render itself by `path` and `metrics`. Renders the full track from project start including all plugins and automation (freeze mode Pre Fader). If the track is already frozen the call fails safely and restores state. TEMPO: with start_bar/end_bar the slice's boundaries are integrated over the project's tempo map, read out of Logic's Tempo List (View > List Editors > Tempo; no playhead movement, and reported in tempo_map). That read costs ~2 s the first time this server sees a project and 27-48 ms every time after — the tempo and meter maps are cached ON DISK per project, so a fresh session is not a cold map. When the Tempo List cannot be read the slice falls back to constant-tempo bar math and the tempo is sampled at both ends of the range instead (parks the playhead, reads, restores), with a `warning` naming both readings when they differ — the FULL render is unaffected either way. Without a bar range no tempo is read at all. MAY RETURN `warning` — read it before the rest of the result (RESULT CONTRACT in the server instructions).
...
READ an existing automation curve before you overwrite it — volume, pan, a send level or any plugin parameter — over a bar range. NOTHING IN THE PROJECT CHANGES: no automation mode is switched, no fader or vpot is moved, and the playhead is put back where it started (`playhead_restored` says whether that worked). It is not free of side effects, though, which is why it is not flagged read-only: it SELECTS the strip it reads, and it HANDS THE SURFACE OVER in the view it read through (Volume, or the send/plugin view) instead of spending 3.3-4.6 s walking home to the Pan view — the next tool that needs Pan settles that itself, and a session that ends pays it at exit. Mechanism: park the playhead at each sampled position and read the value Logic chases the lane to, which is the verification pass logic_record_automation already runs, with the writing half removed. THE POSITION IS PART OF THE READING: every point's bar/beat is the position the park VERIFIED on the control bar and the MCU display CONFIRMED, never the one merely requested — a position the playhead could not reach is listed in omitted_positions with its reason instead of carrying a neighbour's value, a park that landed elsewhere is reported where it landed (with requested_bar/requested_beat), and a first position that cannot be reached refuses the call. The sampling grid takes each bar's beat count from the project's Signature List (reported in meter_map / meter_route), not from the signature at the playhead, so a range crossing a 4/4-to-5/4 boundary asks for positions that exist. HONESTY: these are SAMPLES, not the lane's breakpoints — a move that happens entirely between two samples is invisible, so lower resolution_beats when the shape matters — and an unautomated lane reads as a flat line at the track's static value, which the result says rather than implying a curve exists. automation_mode is the inspector's own label, re-read up to four times before it gives up; when no label is published the result carries automation_mode_unavailable, because a null mode means UNKNOWN and cannot rule out Off. Positions run from start_bar beat 1 to end_bar beat 1 inclusive and end_bar is ALWAYS sampled: a resolution_beats wider than the remaining span shortens the LAST interval (final_interval_beats) rather than dropping the end. Costs roughly a second per sampled position plus ~0.5 s of setup, and up to ~0.13 s per bar of playhead travel EACH WAY when the playhead does not start inside the range (5.1 s each way, measured from 39 bars out). MAY RETURN `warning` — read it before the rest of the result (RESULT CONTRACT in the server instructions).
