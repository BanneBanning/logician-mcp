...
Logician controls Logic Pro through its **data plane**: the Mackie Control surface protocol over virtual MIDI (with Logic's own LCD/LED/fader echoes as verification), MIDI-note-bound key commands, timed MIDI streams for recording, and element-addressed macOS Accessibility for reading structure. **No coordinate-driven UI scripting.** A few verified fallbacks synthesize one of Logic's own shortcuts or press one hit-tested element, and each names its route in the result's `write_route` — trust that field rather than an assumption about how a write happened. (Two tools also offer an explicit `allow_mouse: true` fallback that briefly takes the pointer; it is off by default. ONE route takes the pointer *without* asking, and it says so in `write_route`: `logic_set_track_stack`'s hit-tested click on the disclosure triangle — the only route Logic leaves, since it refuses to learn a key command for its own stack fold. `logic_set_cycle_range` was the second until 2026-09-03: changing a cycle's LENGTH dragged the ruler with synthetic mouse events. It now writes both locators as numbers into the control bar LCD's own locator cells, `write_route` reads `lcd_locator_cells` on every call, and the drag is deleted. A unit test keeps the census of files that synthesize pointer input at two, so a third cannot appear quietly.)
...
Expand or collapse a track stack by pressing its disclosure triangle, verifying the new state, and reporting which subtracks were revealed or hidden. Fails with not_exposed if the track is not a stack. Subtracks of a collapsed stack are otherwise invisible to logic_list_tracks and logic_select_track. THIS IS THE ONE WRITE IN THIS SERVER THAT BRIEFLY TAKES THE POINTER WITHOUT AN allow_mouse OPT-IN, and it is the only route Logic leaves: AXPress, AXShowMenu and AXValue writes on the track-header disclosure are all verified no-ops, and Logic 12.3.1 refuses to LEARN a MIDI note for its own 'Open/Close Track Stack' key command (measured 2026-09-03: four rounds, three candidate notes each, through both learn tools, nothing bound and nothing changed). `write_route` says cg_click_on_ax_frame so you can see it happened. The click is hit-test guarded and refuses rather than clicking blind - and that guard DOES fire in practice (six consecutive refusals measured the same day, with every Accessibility read around it working), so treat a not-exposed-looking failure here as the pointer route being unable to reach the triangle rather than as a missing stack.
