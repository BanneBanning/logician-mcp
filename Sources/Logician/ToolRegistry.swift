    ...
    private static func buildToolRegistry() -> [Tool] {
        ...
        return [
            ...
            Tool(
                ...
                description: "Take a recorded automation curve back OFF a track — the one thing logic_record_automation could not undo, and the reason a ride used to be permanent. DESTRUCTIVE AND TRACK-WIDE: Logic's own command (Mix > Delete Automation > Delete All Automation on Selected Tracks) clears EVERY lane on the addressed track — volume, pan, sends, plugin parameters — so read anything you want to keep with logic_read_automation first and re-record it. AND IT CAN CHANGE THE SOUND: Logic leaves the control where the automation last put it, not where it stood before the curve was written (measured 2026-09-03: a track sitting at -5.1 dB read -2.9 dB once its curve was deleted), so check the static value afterwards with logic_mixer_snapshot. PROVEN, NOT HOPED: the lane you nominate is read before the press (a curve has to actually be there) and read again after (it has to be flat), and the result carries points_before, points_after, value_spread_before/after and verified. A lane that already reads flat comes back as already_empty with NOTHING pressed — a flat reading is what an unautomated lane and a perfectly flat curve both look like from here, and it says nothing about the track's other lanes, so pass force: true when you know better. IT REFUSES RATHER THAN GUESSES. scope 'lane' is refused: Logic's per-lane command deletes whichever lane the automation view is SHOWING, and with that view closed no track header publishes which one, so the press could not be aimed. A bar RANGE is refused: Logic's delete commands take a track, never a range — ride the old value across those bars with logic_record_automation instead, which overwrites the range it rides. scope 'project' is refused: Logic has that command and no readback here could prove what it destroyed. Each refusal names the way round. WHAT IT CAN ADDRESS is a TRACK ROW: unlike logic_record_automation it needs no automation-mode label from the inspector, but Logic's command acts on selected TRACKS, so a mixer-only aux or bus is out of reach and is refused with the rendered rows named (measured 2026-09-03: the sandbox's `Aux 1` has no track row, and its lane could not be cleared this way). logic_list_tracks is the list of what it can address. Two things it does first, both of them guards: the track selection is narrowed to that ONE row (the menu item says Selected Tracks and means it), and Logic is brought to the front (a menu press from the background answers success and does nothing — measured 2026-09-03). An unrecognised dialog is CANCELLED, never answered. COST, measured 2026-09-03 on the sandbox: 9.1 s for a verified removal proved at three sampled positions, of which the menu press is 0 ms and essentially all the rest is the two verification reads (~1 s a sampled position plus the playhead travel to reach them); 3.6 s for the already_empty answer, which pays for one read and no press. verify_resolution_beats defaults to one sample a bar because presence-or-absence needs far less resolution than shape.",
                inputSchema: [
                    "type": "object",
                    "properties": [
            ...
            Tool(
                ...
                inputSchema: [
                    ...
                    "properties": [
                        ...
                        "verify_render": ["type": "boolean", "description": "Default true: freeze-render the recorded bars afterwards and return slice metrics as proof. The render moves the playhead again (freeze jumps it to the project start and rolls from there) and restores it afterward the same way the take itself does - its outcome is what `playhead` in the result describes, since it is the last thing to touch the control bar."],
                        ...
                        "tracks": [
                            ...
                            "items": [
                                ...
                                "properties": [
                                    ...
                                    "to_track": [
                                        "type": "string",
                                        "description": "COMPOSE ONTO YOUR OWN TRACK: the name of an EXISTING track this part should end up on, so it plays through that track's instrument instead of the default patch Logic picks for a new one. Per track, not per call - route drums onto 'Drums' and bass onto 'Bas' in the same import, and leave it out on any entry that should keep a new track of its own. Costs ~2.6 s per routed track (measured back to back against the same import unrouted: 5.8 s vs 3.3 s): the import still lands on a temp track, then the region is moved onto the destination (~1.4 s) and the emptied temp track is deleted (~1 s, plus the re-read census). Destinations are resolved BEFORE anything is imported, so a name that does not exist is refused with the project untouched; an output/aux/bus strip is refused too (a region needs a track lane)."
                                    ],
                                    "to_track_number": [
                                        "type": "integer",
                ...
                inputSchema: [
                    ...
                    "properties": [
                        ...
                        "commands": [
                            ...
                            "description": "Limit to these command names (default: the 20 the install round writes). Undo, Redo and Flashback Capture as Recording are accepted here but are NOT in that default - nothing fires them, so naming one is how you install it deliberately."
                        ]
                    ],
                    "additionalProperties": false
                ],
                ...
                inputSchema: [
                    ...
                    "properties": [
                        ...
                        "hold_ms": [
                            ...
                            "description": "How long a press/select/mute/solo/vpot_press holds the button down. Default 0, and 0 is the MEASURED default: holds from ~0.2 ms to 50 ms were all honoured, 16 of 16, with the same 102-106 ms echo. Pass one only for Logic Control's hold behaviours, which are unswept. On keycmd this is the same field, now ALSO measured and defaulting to 0: firing Create Marker at hold_ms 0, 1, 5, 10, 20 and 40 (2026-09-03) created exactly one marker every time, 16 fires at 0 ms among them, zero duplicates, zero drops. That covers 0-40 ms; the ~0.2 ms point the button sweep reached is not reachable through this integer field. Pass one only for Logic Control's own hold behaviours, which remain unswept on this plane too."
                        ],
            ...
            Tool(
                ...
                description: "Read and WRITE the project's tempo map — the tempo track, as rows in Logic's Tempo List. This is what makes a tempo map buildable: 'land a downbeat on the hit at 1:12' is create-an-event, and until now the map could only be read. action 'list' returns every event (bar, beat, BPM) and writes nothing; the map is cached per project, every cache hit is cross-checked against the control bar's live tempo, and a list whose cross-check could NOT run (the control bar unreadable, e.g. a non-English Logic UI) comes back verified: false with read_route 'tempo_list_cache' and a warning - never as a verified live read. 'create' makes a new event at bar/beat with bpm; 'set' retunes the event already there; 'delete' removes it. HOW THE WRITE WORKS, because two parts of it are surprising. Logic's own 'Create new Tempo Event' button places the event AT THE PLAYHEAD, not on a bar line, and parking the playhead only sets its bar and beat — the division and tick it already carried come along — so this tool creates the event and then steps the new row's own position fields back to the exact bar/beat you asked for. And the BPM is stepped on the ROW's own tempo cell, one BPM per write, NOT through the control bar's tempo slider — on a project that already has a map, that slider answers a tempo write with a modal alert ('Multiple Tempo Events detected!') that freezes the whole Accessibility plane until someone clicks OK. Neither half is trusted: the whole map is re-read afterwards and the result reports it, so a create that produced two events, or moved a neighbour, comes back as a failure (or a warning naming which other events moved) rather than as a success. Compare-and-set with expected_current_bpm on 'set' and 'delete'. REFUSED: 'create' where an event already sits (use 'set'), 'set'/'delete' where none does, deleting bar 1 (the project's base tempo — retune it instead), and any write at all while the Tempo List cannot be read, because a tempo write is not made blind. COST: 'list' is ~7 ms from the per-project cache and 0.4-0.8 s when it has to read the list; a write is a couple of seconds, dominated by reading the whole Tempo List twice (once before, once to verify) rather than by the playhead, which now travels to any bar in milliseconds. The whole edit is held inside ONE List Editors pane cycle instead of the three or four it used to open and close, which roughly halved every write (measured 2026-09-03). AFTER A WRITE the server's cached tempo map is dropped, so every later bar->seconds conversion re-reads the new map; the cached METER map is left alone, because a tempo event is not a time signature and re-reading the Signature List cost the next logic_list_signatures a full second for nothing.",
                inputSchema: [
                    "type": "object",
                    "properties": [
                        "action": [
            ...
            Tool(
                ...
                description: "The whole mixer in ONE call, off Logic's own control-surface feedback: per strip the fader value in dB, pan, mute/solo/select/record-arm state and the raw 14-bit fader echo. volume_db is the dB string Logic paints in its channel-strip Volume view — the same readout logic_set_track_mix converges against — NOT a conversion of the fader position, which is reported separately and raw as fader_14bit; a cell that does not parse comes back null with the LCD text beside it, never interpolated."
                    + " EVERY LED ANSWER IS A WINDOW, NOT AN INSTANT, because Logic uses a BLINKING LED as a state of its own. An armed strip's record LED flashes (~640 ms on / 640 ms off, measured), so seen-lit-once means armed. The mute LED flashes on every channel a STANDING SOLO silences, so only a steady mute LED is a mute: a blinking one is reported muted: false and marked mute_led_blinking — that strip is silent right now but not muted, and pressing mute on it would mute it. any_soloed is Logic's whole-project solo indicator (note 0x73), which also sees a soloed channel that has no strip on this surface."
                    + " Two bank walks (names and pan, then dB and LEDs), 8.5-10 s on a 25-strip project (measured 2026-09-02; 10-11 s with a solo standing, which costs a banner fade), or 5.5 s with include_record_arm: false when no solo is standing. The surface is HANDED OVER in the channel-strip Volume view (assignment_after says which view, surface_restore says the return to the pan view is deferred): the next control-surface tool that needs the pan view restores it, as does the end of the session — the 3.7 s restore is no longer charged to this call, which had already read every byte it reports."
