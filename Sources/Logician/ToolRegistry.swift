...
extension MCPServer {
    ...
    /// the same values as the first. It was doing exactly that — measured
    ...
    private static func buildToolRegistry() -> [Tool] {
        ...
        return [
            ...
            Tool(
                ...
                title: "Find a tool",
                description: "Which tool does X? Ask this first - 0.4-1.0 ms, no contact with Logic - before reading a schema file, writing your own probe, or reaching for logic_mcu_command's raw escape hatch. Search every tool this server has by keyword and get the matching typed definitions back: name, description, full input schema and safety annotations. Use it when you know what you want to do but not which tool does it, or when a tool you expected is not being offered. The search covers the whole registry even when `--toolsets` narrows a session, and each match says which toolsets hold it and whether it is active here.",
                inputSchema: [
                    "type": "object",
                    "properties": [
                        "query": [
            ...
            Tool(
                ...
                inputSchema: [
                    ...
                    "properties": [
                        ...
                        "allow_mouse": ["type": "boolean", "description": "Permit the synthetic-click fallback if the mouse-free Accessibility press does not fold the stack. Default false: the call refuses instead and names this argument."],
                        "expected_project_path": MCPServer.expectedProjectPathStrictProperty
                    ],
