...
extension LogicAccessibility {
    ...
    func convergeSlider(in controlBar: AXUIElement, sliderName: String, target: Int) throws {
        ...
        try convergeLCDCell(in: lcd, sliderName: sliderName, target: target, label: "playhead")
    }

    /// The same stepper against ANY of the control bar LCD's cells — the
    /// playhead position display, and the two cycle locator cells
    /// `setCycleRange` writes (measured 2026-09-03: same one-step-per-write
    /// contract, 0.1–1.6 ms per step). `convergeSlider` above is this with the
    /// playhead group looked up for it, so the 16 call sites that inherit its
    /// cost inherit exactly the same loop.
    func convergeLCDCell(
        in group: AXUIElement, sliderName: String, target: Int, label: String
    ) throws {
        guard let slider = children(of: group).first(where: {
            ...
            throw LogicianError.windowNotFound("\(label) \(sliderName) slider")
        }
